...
public:
    ...
    string find_first_binary(int index, string value, vector<string> type, long int & initial_address, long int & final_address, vector<int> deleted); //encontra primeira ocorrência do elemento "query"
    ...
    long int find_hash(int key, string fl, vector<tuple<int, int>> deleted, long int &initial, long int &final);
